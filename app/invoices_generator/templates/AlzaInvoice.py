...
class AlzaInvoice(InvoiceTemplate):
    ...
    def render(textRenderer:TextRenderer, data: InvoiceData, invoice:Invoice) -> bool:
        ...
        textRenderer._text(invoice, d,(margin_l, y), text=safe(data.supplier.name), font=textRenderer._f16b, fill=INK)

        # Titul (centrovaný blok vpravo části)
        title_center_x = _A4_W_PX // 2
        textRenderer._text_center(invoice, d, title_center_x, y, text=f"{safe(data.invoice_number)}", font=textRenderer._f17b, fill=INK, label="Faktura -", span_tag=SpanTag.INVOICE_NUMBER)
        textRenderer._text_center(invoice, d, title_center_x, y + mm(12), "záruční a dodací list -", textRenderer._f12, MUTED)

        y += mm(18)
        local_x = margin_l
        # --- Prodávající ---
        textRenderer._text(invoice, d,(margin_l, y), text=f"Prodávající: {safe(data.supplier.name)} {data.supplier.type.value}", font=textRenderer._f12b,
                fill=INK)
        
        y += mm(5.2)
        local_x, _ = textRenderer._text(invoice, d,(local_x, y),
        ...
        # Levý blok
        textRenderer._text(invoice, d,(left_x, y), text="Daňový doklad:", font=textRenderer._f12b, fill=INK)
        y_left = y + mm(6)

        kv_label_w = mm(60)

        def kv_row(x:int, y_:int, label:str, value:str, bold:bool=True, tag: SpanTag = SpanTag.O, undersampling:bool = True)->None:
            textRenderer._text(invoice, d,(x, y_), text=label, font=textRenderer._f11, fill=INK, span_tag=SpanTag.O)
            fontv = textRenderer._f11b if bold else textRenderer._f11
            textRenderer._text(invoice, d,(x + kv_label_w, y_), text=value, font=fontv, fill=INK, span_tag=tag)
        ...
        textRenderer._text(invoice, d,(left_x, y_left + mm(2)), text="Bankovní účet:", font=textRenderer._f12b, fill=INK)
        y_left += mm(8)
        ...
        # Pravý blok
        textRenderer._text(invoice, d,(right_x, y), text="Kupující:", font=textRenderer._f12b, fill=INK)
        y_right = y + mm(6)
        ...
        def kv_r(label:str, value:str, tag: SpanTag = SpanTag.O, undersampling:bool = True)->None:
            textRenderer._text(invoice, d,(inner_x, y_tmp), text=label, font=textRenderer._f11, fill=INK)
            textRenderer._text(invoice, d,(inner_x + mm(40), y_tmp), text=safe(value), font=textRenderer._f11b, fill=INK, span_tag=tag)
        ...
        for it in data.items:
            ...
            textRenderer._text_right(invoice, d, x_cols[5] + col_abs[5] - 6, y_text, cells[5], textRenderer._f11, INK)
            textRenderer._text_right(invoice, d, x_cols[6] + col_abs[6] - 6, y_text, cells[6], textRenderer._f11, INK)
        ...
        textRenderer._text(invoice, d,(margin_l + 6, y + mm(2.5)), "Celkem:", font=textRenderer._f11b, fill=INK)
        total_txt = f"{fmt_money(data.calculated_total_price)}"
        textRenderer._text_right(invoice, d, margin_l + table_w - 6, y + mm(2.5), total_txt, textRenderer._f11b, INK,end=f"{data.currency.value if hasattr(data.currency, 'value') else data.currency}", span_tag=SpanTag.TOTAL)
        y += foot_h
        ...
        textRenderer._text(invoice, d,(box_x + mm(6), y + mm(3)), "Vyčíslení DPH:", font=textRenderer._f12b, fill=INK)
        head_y = y + mm(9)
        textRenderer._text_center(invoice, d, box_x + box_w * 0.16, head_y, "Sazba", textRenderer._f11b, INK)
        textRenderer._text_center(invoice, d, box_x + box_w * 0.50, head_y, "Základ", textRenderer._f11b,INK)
        textRenderer._text_center(invoice, d, box_x + box_w * 0.84, head_y, "DPH", textRenderer._f11b, INK)
        d.line((box_x + 4, head_y + mm(4), box_x + box_w - 4, head_y + mm(4)), fill=LINE_STRONG,
                width=3)

        row_y = head_y + mm(6.5)
        for v in data.vat:
            _, percentage_id = textRenderer._text_center(invoice, d, box_x + box_w * 0.16, row_y, text=f"{safe(v.vat_percentage)}", end=" %", font=textRenderer._f11, fill=INK, span_tag=SpanTag.O)
            _, base_id = textRenderer._text_right(invoice, d, box_x + box_w * 0.66, row_y, fmt_money(v.vat_base), textRenderer._f11, INK, span_tag=SpanTag.O)
            _, vat_id = textRenderer._text_right(invoice, d, box_x + box_w - mm(6), row_y, fmt_money(v.vat), textRenderer._f11, INK, span_tag=SpanTag.O)
        ...
        textRenderer._text(invoice, d,(right_block_x + mm(25), y + mm(2)),
                f"Zaokrouhlení: 0,00 {data.currency.value if hasattr(data.currency, 'value') else data.currency}",
                font=textRenderer._f11, fill=INK)
        textRenderer._text(invoice, d,(right_block_x + mm(25), y + mm(2) + mm(6)),
        ...
        # --- PATIČKA ---
        textRenderer._text(invoice, d,(margin_l, y), "Poznámka:", font=textRenderer._f11, fill=INK)
        y += mm(10)
        ...
        textRenderer._text_center(invoice, d, qr_x + qr_size / 2, qr_y + qr_size / 2 - mm(2), "QR", textRenderer._f10, (170, 170, 170))

        # Spodní lišta
        bar_y = qr_y + mm(22) + mm(8)
        hr(bar_y, "thin")
        textRenderer._text(invoice, d,(margin_l, bar_y + mm(2)), "Ochranný znak …", font=textRenderer._f11, fill=INK)
        textRenderer._text_center(invoice, d, _A4_W_PX / 2, bar_y + mm(2), "Strana 1 z 1", textRenderer._f11, INK)
        now_str = datetime.now().strftime("%d.%m.%Y %H:%M")
        textRenderer._text_right(invoice, d, _A4_W_PX - margin_r, bar_y + mm(2), f"Tisk: {now_str}", textRenderer._f11, INK)
