...
class HeaderB(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, d, (x, y), text=safe(data.supplier.name).upper(), font=textRenderer._f12b, fill=INK)
        y += mm(6)
        textRenderer._text(invoice, d, (x, y), text="SPECIALIZOVANÝ MALOOBCHOD A SERVIS", font=textRenderer._f10, fill=TMOBILE_PINK)
        ...
        textRenderer._text(invoice, d, (x, y), text=legal_text, font=textRenderer._f10, fill=MUTED)
        ...
        textRenderer._text(invoice, d, (r_align, y), text="ID datové schránky: 8t5sa2q", font=textRenderer._f10, fill=MUTED)
        y += mm(3.5)
        textRenderer._text(invoice, d, (r_align, y), text="Provozovna: Průmyslová 14, 102 00 Praha 10", font=textRenderer._f10, fill=MUTED)
        
        # 6. "Nedůležité" metadata (vlevo)
        y -= mm(0) # mírná korekce výšky
        textRenderer._text(invoice, d, (x, y), text="Certifikace: ISO 9001:2015 Management kvality", font=textRenderer._f10, fill=MUTED)
