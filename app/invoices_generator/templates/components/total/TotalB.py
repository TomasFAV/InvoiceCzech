...
class TotalB(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, d, (x, y), text="CELKOVÁ ČÁSTKA K ÚHRADĚ", font=textRenderer._f8b, fill=BG)
        ...
        textRenderer._text(invoice, d, (x, y), text=total_txt, end=f"{data.currency.value}", font=textRenderer._f18b, fill=BG, span_tag=SpanTag.TOTAL)
        
        # 4. DOPLNĚK (Např. kurz nebo poznámka pod čarou v rámci boxu)
        textRenderer._text(invoice, d, (x, y + mm(8)), text="Včetně DPH a všech poplatků", font=textRenderer._f8, fill=SUBTLE_BG)
