...
class InfoC(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, d, (x, y), text="Doklad č.", font=textRenderer._f13, fill=MUTED)
        # 1. Číslo faktury s výrazným podtržením
        textRenderer._text(invoice, d, (x + mm(45), y), text=safe(data.invoice_number), 
        ...
        for attr, lab, tag in fields:
            ...
            if val:
                # Popisek vlevo
                textRenderer._text(invoice, d, (x, y), text=lab, font=textRenderer._f13, fill=MUTED)
                # Hodnota vpravo (zarovnaná k fixnímu bodu)
                textRenderer._text(invoice, d, (x + mm(45), y), text=val, font=textRenderer._f14b, fill=INK, span_tag=tag)
        ...
        textRenderer._text(invoice, d, (x + mm(4), y), text="SPLATNOST DO:", font=textRenderer._f13b, fill=INK)
        textRenderer._text(invoice, d, (x + mm(45), y), text=safe(data.due_date), 
