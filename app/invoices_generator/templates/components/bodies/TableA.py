...
from invoices_generator.utility.invoice_consts import INK, LINE, LINE_STRONG
from invoices_generator.utility.utils import get_item_value
...
class TableA(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        for i, col in enumerate(config):
            textRenderer._text(invoice, d, (x_cols[i] + width*0.05, y + row_height*0.05), col[0], font=textRenderer._f12b, fill=INK)
        y += row_height
        ...
        for idx, it in enumerate(data.items):
            ...
            for i, col in enumerate(config):
                ...
                textRenderer._text(invoice, d, (x_cols[i] + width*0.05, y + row_height*0.05), val, font=textRenderer._f12, fill=INK)
