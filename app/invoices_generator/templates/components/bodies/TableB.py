...
from invoices_generator.utility.invoice_consts import INK, MUTED, LINE, LINE_STRONG, SUBTLE_BG
from invoices_generator.utility.utils import fmt_money, get_item_value
...
class TableB(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        for i, col in enumerate(config):
            ...
            if i == 0:
                textRenderer._text(invoice, d, (x_cols[i] + width*0.05, y + row_height*0.05), col[0], font=scaled_f10b, fill=MUTED)
            else:
                textRenderer._text_right(invoice, d, x_cols[i] + col_widths[i] - width*0.05, y + row_height*0.05, col[0], font=scaled_f10b, fill=MUTED)
        ...
        for idx, it in enumerate(data.items):
            ...
            for i, col in enumerate(config):
                ...
                if i == 0:
                    ...
                    textRenderer._text(invoice, d, (x_cols[i] + width*0.05, y), fmt_money(val), font=scaled_f9b, fill=INK)
                else:
                    # Ostatní hodnoty normálně
                    textRenderer._text_right(invoice, d, x_cols[i] + col_widths[i] - width*0.05, y, fmt_money(val), font=scaled_f9, fill=INK)
            
            y += row_height*0.4
            
            # Sub-text (vytvoříme vatu: kód produktu nebo popis dph)
            textRenderer._text(invoice, d, (x_cols[0] + width*0.05, y), f"Kód: {random.randint(1000,9999)}", font=scaled_f8, fill=MUTED)
        ...
        textRenderer._text_right(invoice, d, x + table_w - width*0.2, y, total_label, font=scaled_f10b, fill=INK)
        textRenderer._text_right(invoice, d, x + table_w - width*0.05, y - row_height*0.05, text=total_val, end="Kč", font=scaled_f13b, fill=INK, span_tag=SpanTag.TOTAL)
