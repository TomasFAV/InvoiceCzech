    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        for word in words:
            if text_width(line + word, textRenderer._f8) < mm(55):
                ...
                textRenderer._text(invoice, d, (text_x, text_y), text=line, font=textRenderer._f8, fill=INK)
                text_y += mm(3)
                line = word + " "
        textRenderer._text(invoice, d, (text_x, text_y), text=line, font=textRenderer._f8, fill=INK)
        ...
        textRenderer._text(invoice, d, (cert_x + mm(2), cert_y + mm(2)), text="DIGITALLY SIGNED", font=textRenderer._f8b, fill=INK)
        textRenderer._text(invoice, d, (cert_x + mm(2), cert_y + mm(6)), text="Verified by Auth-ID", font=textRenderer._f8, fill=MUTED)
        ...
        textRenderer._text(invoice, d, (x, y), text=fine_print, font=textRenderer._f8, fill=MUTED)
