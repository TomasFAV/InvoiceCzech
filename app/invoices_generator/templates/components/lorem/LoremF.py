...
class LoremF(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        for title, text in selected:
            ...
            textRenderer._text(invoice, d, (x + icon_size + mm(2), y), text=title, font=textRenderer._f8b, fill=INK)
            y += mm(4.5)
            ...
            for word in words:
                if text_width(line + word, textRenderer._f8) < (max_w - mm(5)):
                    ...
                    textRenderer._text(invoice, d, (x + mm(5), y), text=line.strip(), font=textRenderer._f8, fill=MUTED)
                    y += mm(3.2)
                    line = word + " "
        ...
        textRenderer._text(invoice, d, (x, y), text=sys_code, font=textRenderer._f8, fill=MUTED)
