...
class BankDetailsB(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, d, (curr_x, curr_y), text="ČÍSLO ÚČTU", font=textRenderer._f10b, fill=MUTED)
        textRenderer._text(invoice, d, (curr_x, curr_y + mm(4)), 
        ...
        if not is_narrow:
            textRenderer._text(invoice, d, (curr_x, curr_y + mm(9)), text="Banka: Centrální Komerční Banka, a.s.", font=textRenderer._f12, fill=MUTED)
        ...
        textRenderer._text(invoice, d, (curr_x, curr_y), text="IBAN / BIC", font=textRenderer._f12b, fill=MUTED, must_have_same_width=True)
        
        iban_val = safe(data.IBAN)
        # Pokud je IBAN moc dlouhý a jsme v úzkém režimu, můžeme ho zkrátit nebo zmenšit
        textRenderer._text(invoice, d, (curr_x, curr_y + mm(4)), text=iban_val, 
                 font=textRenderer._f13b if not is_narrow else textRenderer._f14b, fill=INK, span_tag=SpanTag.IBAN)
        
        if not is_narrow:
            textRenderer._text(invoice, d, (curr_x, curr_y + mm(9)), 
                     text="Měna: CZK | Clearing: DOMESTIC", font=textRenderer._f12, fill=MUTED)
        else:
            # V úzkém režimu dáme BIC vedle IBANu nebo pod něj
            textRenderer._text(invoice, d, (curr_x, curr_y + mm(8)), label="BIC: ", text=f"{safe(data.bank_account.BIC)}", font=textRenderer._f12b, fill=INK, span_tag=SpanTag.BIC)
