...
class BankDetailsC(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        if is_compact:
            # --- ÚZKÝ REŽIM (70 mm) ---
            # Banka
            textRenderer._text(invoice, d, (curr_x, curr_y), text="BANKA", font=textRenderer._f12b, fill=MUTED)
            textRenderer._text(invoice, d, (curr_x + mm(30), curr_y), text="Centrální Komerční Banka", font=textRenderer._f12, fill=INK)
            
            # Číslo účtu
            curr_y += mm(6)
            textRenderer._text(invoice, d, (curr_x, curr_y), text="ÚČET", font=textRenderer._f12b, fill=MUTED)
            textRenderer._text(invoice, d, (curr_x + mm(30), curr_y - mm(0.5)), text=safe(data.bank_account_number), 
                     font=textRenderer._f14b, fill=INK, span_tag=SpanTag.BANK_ACCOUNT_NUMBER)
            
            # IBAN
            curr_y += mm(7)
            textRenderer._text(invoice, d, (curr_x, curr_y), text="IBAN", font=textRenderer._f12b, fill=MUTED)
            textRenderer._text(invoice, d, (curr_x + mm(30), curr_y), text=safe(data.IBAN), 
                     font=textRenderer._f12b, fill=INK, span_tag=SpanTag.IBAN)
            
            # BIC
            curr_y += mm(5)
            textRenderer._text(invoice, d, (curr_x, curr_y), text="BIC", font=textRenderer._f12b, fill=MUTED)
            textRenderer._text(invoice, d, (curr_x + mm(30), curr_y), text=safe(data.bank_account.BIC), 
            ...
            # Col 1: Banka
            textRenderer._text(invoice, d, (curr_x, curr_y), text="BANKOVNÍ SPOJENÍ", font=textRenderer._f12b, fill=MUTED)
            textRenderer._text(invoice, d, (curr_x, curr_y + mm(5)), text="Centrální Komerční Banka", font=textRenderer._f13, fill=INK)
            
            # Col 2: Účet
            textRenderer._text(invoice, d, (x + col_w, curr_y), text="ČÍSLO ÚČTU", font=textRenderer._f12b, fill=MUTED)
            textRenderer._text(invoice, d, (x + col_w, curr_y + mm(4.5)), text=safe(data.bank_account_number), 
                     font=textRenderer._f13b, fill=INK, span_tag=SpanTag.BANK_ACCOUNT_NUMBER)
            
            # Col 3: IBAN/BIC
            textRenderer._text(invoice, d, (x + 2*col_w, curr_y), text="IBAN / BIC", font=textRenderer._f12b, fill=MUTED, must_have_same_width=True)
            textRenderer._text(invoice, d, (x + 2*col_w, curr_y + mm(5)), text=f"{safe(data.IBAN)}", font=textRenderer._f12b, fill=INK, span_tag=SpanTag.IBAN)
