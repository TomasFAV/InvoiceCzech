...
class CompanyA(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, d, (x, y), text="Dodavatel:" if supplier else "Zákazník:", font=scaled_f12b, fill=INK)

        y += space + mm(1)
        textRenderer._text(invoice, d, (x, y), label="", text=safe(values.name), font=scaled_f11, fill=INK)
        y += space
        textRenderer._text(invoice, d, (x, y), label="adresa:", text=safe(values.address), font=scaled_f10, fill=INK)
        y += space
        
        if random.random() > 0.1:
            if(supplier):
                textRenderer._text(invoice, d, (x, y), label="IČ:", text=safe(values.register_id), font=scaled_f10, fill=INK, span_tag=SpanTag.SUPPLIER_REGISTER_ID)
                y += space
            else:
                textRenderer._text(invoice, d, (x, y), label="IČ:", text=safe(values.register_id), font=scaled_f10, fill=INK, span_tag=SpanTag.CUSTOMER_REGISTER_ID)
                y += space


        if random.random() > 0.1:
            if(supplier):
                textRenderer._text(invoice, d, (x, y), label="DIČ:", text=safe(values.tax_id), font=scaled_f10, fill=INK, span_tag=SpanTag.SUPPLIER_TAX_ID)
            else:
                textRenderer._text(invoice, d, (x, y), label="DIČ:", text=safe(values.tax_id), font=scaled_f10, fill=INK, span_tag=SpanTag.CUSTOMER_TAX_ID)
