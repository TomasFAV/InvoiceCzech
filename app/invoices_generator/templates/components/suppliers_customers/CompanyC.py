...
class CompanyC(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, d, (x + mm(2), y + mm(0.5)), text=label_text, font=scaled_f8b, fill=MUTED)
        ...
        textRenderer._text(invoice, d, (x, y), text=company_name, font=scaled_f10b, fill=INK)
        ...
        for part in addr_parts:
            adress_end_x = max(textRenderer._text(invoice, d, (x, y), text=part.strip(), font=scaled_f8, fill=INK)[0], adress_end_x)
            y += space
        ...
        if (supplier):
            textRenderer._text(invoice, d, (x, y), label="IČ", text=safe(values.register_id), font=scaled_f9, fill=MUTED,
                      span_tag=SpanTag.SUPPLIER_REGISTER_ID)
            y += space
            textRenderer._text(invoice, d, (x, y), label="DIČ", text=safe(values.tax_id), font=scaled_f9, fill=MUTED,
