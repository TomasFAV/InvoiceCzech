...
from invoices_generator.utility.utils import (
...
from invoices_generator.utility.invoice_consts import INK, MUTED, TMOBILE_PINK


class company_b(invoice_component):
    ...
    def draw(inv: DInvoice, d: ImageDraw, x: int, y: int, **kwargs):
        ...
        scaled_f11b = truetype(inv._f11b.path, int(max_font_size))
        scaled_f10 = truetype(inv._f10.path, int(max_font_size) - 1)
        scaled_f9  = truetype(inv._f9.path,  int(max_font_size) - 2)
        scaled_f8b = truetype(inv._f8b.path, int(max_font_size) - 3)
        ...
        # 2) Název firmy
        inv._text(d, (x, y), text=safe(values.name).upper(), font=scaled_f11b, fill=INK)
        ...
        address_x = x + mm(4)
        inv._text(d, (address_x, y), text=safe(values.address), font=scaled_f10, fill=INK)
        y += space

        # IČ / DIČ se span tagy dle role
        reg_tag = span_tags.SUPPLIER_REGISTER_ID if supplier else span_tags.CUSTOMER_REGISTER_ID
        tax_tag = span_tags.SUPPLIER_TAX_ID if supplier else span_tags.CUSTOMER_TAX_ID
