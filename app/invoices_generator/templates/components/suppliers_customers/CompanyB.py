...
class CompanyB(InvoiceComponent):    
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        # 2) Název firmy
        textRenderer._text(invoice, d, (x, y), text=safe(values.name).upper(), font=scaled_f11b, fill=INK)
        ...
        address_x = x + mm(4)
        textRenderer._text(invoice, d, (address_x, y), text=safe(values.address), font=scaled_f10, fill=INK)
        y += space
