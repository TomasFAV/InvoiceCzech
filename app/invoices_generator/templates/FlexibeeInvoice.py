...
class FlexibeeInvoice(InvoiceTemplate):
    ...
    def render(textRenderer:TextRenderer, data: InvoiceData, invoice:Invoice) -> bool:
        ...
        # --- HLAVIČKA ---
        textRenderer._text(invoice,d, (margin_l, y), "Faktura - daňový doklad", font=textRenderer._f18b, fill=INK)
        # číslo vpravo nahoře
        ...
        lx = margin_l + mm(4)
        ly = main_top + mm(4)
        textRenderer._text(invoice,d, (lx, ly), "Dodavatel:", font=textRenderer._f11, fill=INK)
        ly += mm(6)
        ...
        if getattr(data.supplier, "country", None):
            textRenderer._text(invoice,d, (lx, ly), safe(data.supplier.country.name), font=textRenderer._f12, fill=INK)
            ly += mm(6)
        ...
        ly += mm(4)
        textRenderer._text(invoice,d, (lx, ly), "E-mail:", font=textRenderer._f10, fill=MUTED)
        ...
        ly += mm(4.2)
        textRenderer._text(invoice,d, (lx, ly), "WWW:", font=textRenderer._f10, fill=MUTED)
        ...
        textRenderer._text(invoice,d, (rx, ry), "Odběratel - sídlo:", font=textRenderer._f11, fill=INK)
        ...
        textRenderer._text(invoice,d, (rx, post_y0 + mm(4)), "Poštovní adresa:", font=textRenderer._f11, fill=INK)
        ...
        textRenderer._text(invoice,d, (rx, low_y0 + mm(4)), "Místo určení:", font=textRenderer._f11, fill=INK)
        # další řádky
        textRenderer._text_right(invoice,d, rx + mm(40), low_y0 + mm(10), text=safe(data.issue_date), font=textRenderer._f11, fill=INK, span_tag=SpanTag.ISSUE_DATE, label="Vystaveno: ")
        textRenderer._text_right(invoice,d, rx + mm(40), low_y0 + mm(15), text=safe(data.due_date), font=textRenderer._f11b, fill=INK, span_tag=SpanTag.DUE_DATE, label="Datum splatnosti: ")
        textRenderer._text_right(invoice,
            d,
            rx+mm(40),
            low_y0 + mm(20),
        ...
        textRenderer._text(invoice,d, (blx, bly), "Banka:", font=textRenderer._f11, fill=INK)
        ...
        bly += mm(5)
        textRenderer._text(invoice,d, (blx, bly), "Bankovní účet:", font=textRenderer._f11, fill=INK)
        # rámeček s účtem
        ...
        bly += mm(6)
        textRenderer._text(invoice,d, (blx, bly), "IBAN:", font=textRenderer._f11, fill=INK)
        textRenderer._text(invoice,
            d,
            (blx + mm(34), bly),
        ...
        bly += mm(5)
        textRenderer._text(invoice,d, (blx, bly), "BIC:", font=textRenderer._f11, fill=INK)
        textRenderer._text(invoice,
            d,
            (blx + mm(30), bly),
        ...
        sym_y = bly + mm(6)
        textRenderer._text(invoice,d, (blx, sym_y), "Var. sym.:", font=textRenderer._f11, fill=INK)
        textRenderer._text(invoice,
            d,
            (blx + mm(30), sym_y),
        ...
        sym_y += mm(5)
        textRenderer._text(invoice,d, (blx, sym_y), "Konst. sym.:", font=textRenderer._f11, fill=INK)
        textRenderer._text(invoice,
            d,
            (blx + mm(30), sym_y),
        ...
        sym_y += mm(5)
        textRenderer._text(invoice,d, (blx, sym_y), "Spec. sym.:", font=textRenderer._f11, fill=INK)
        textRenderer._text(invoice,
            d,
            (blx + mm(30), sym_y),
        ...
        textRenderer._text(invoice,d, (blx, pay_y), "Forma úhrady:", font=textRenderer._f11, fill=INK)
        textRenderer._text(invoice,
            d,
            (blx + mm(34), pay_y),
        ...
        textRenderer._text(invoice,d, (blx, pay_y + mm(5)), "Způsob dopravy:", font=textRenderer._f11, fill=INK)
        ...
        for i, h in enumerate(headers):
            if i == 0:
                textRenderer._text(invoice,d, (x_cols[i], y), h, font=textRenderer._f11b, fill=INK, must_have_same_width=True)
            else:
                textRenderer._text_center(invoice,d, x_cols[i] + col_w[i] / 2, y, h, textRenderer._f11b, INK, must_have_same_width=True)
        y += mm(5)
        hr(y, "thin")
        y += mm(2)
        ...
        for it in data.items:
            ...
            # 0 popis
            textRenderer._text(invoice,d, (x_cols[0], ty), safe(it.description), font=textRenderer._f11, fill=INK)
            ...
            # 3 cena za MJ
            textRenderer._text_right(invoice,d, x_cols[3] + col_w[3], ty, fmt_money(it.ppu), textRenderer._f11, INK)
            # 4 sazba DPH
            textRenderer._text_center(invoice,
                d,
                x_cols[4] + col_w[4] / 2,
            ...
            # 5 základ
            textRenderer._text_right(invoice,d, x_cols[5] + col_w[5], ty, fmt_money(it.price_without_vat), textRenderer._f11, INK)
            # 6 DPH
            textRenderer._text_right(invoice,d, x_cols[6] + col_w[6], ty, fmt_money(it.vat), textRenderer._f11, INK)
            # 7 celkem
            textRenderer._text_right(invoice,d, x_cols[7] + col_w[7], ty, fmt_money(it.price_with_vat), textRenderer._f11, INK)
        ...
        textRenderer._text_right(invoice,d, total_base_x - mm(6), y, "Celkem:", textRenderer._f11b, INK)
        # základ/dph/celkem
        if len(data.vat) > 0:
        ...
        textRenderer._text_right(invoice,d, total_base_x + col_w[5], y, fmt_money(base_total), textRenderer._f11b, INK, span_tag=SpanTag.TOTAL_BASE if hasattr(SpanTag, "TOTAL_BASE") else SpanTag.O)
        textRenderer._text_right(invoice,d, total_vat_x + col_w[6], y, fmt_money(vat_total), textRenderer._f11b, INK, span_tag=SpanTag.TOTAL_VAT if hasattr(SpanTag, "TOTAL_VAT") else SpanTag.O)
        textRenderer._text_right(invoice,d, total_sum_x, y, fmt_money(data.calculated_total_price), textRenderer._f11b, INK, span_tag=SpanTag.TOTAL)
        ...
        textRenderer._text(invoice,d, (recap_x0, y), "Rekapitulace DPH v Kč", font=textRenderer._f11, fill=INK)
        y_re = y + mm(5)
        ...
        for v in data.vat:
            # "Základ 21% .... DPH 21% ...."
            textRenderer._text(invoice,d, (recap_x0, y_re), "Základ", font=textRenderer._f10, fill=INK)
            textRenderer._text(invoice,
                d,
                (recap_x0 + mm(22), y_re),
                ...
                fill=INK,
                span_tag=SpanTag.O)
            textRenderer._text(invoice,d, (recap_x0 + mm(48), y_re), label="DPH", text=f"{safe(v.vat_percentage)}", end="%", span_tag=SpanTag.O, font=textRenderer._f10, fill=INK)
            textRenderer._text_right(invoice,
                d,
                recap_x1,
                y_re,
        ...
        textRenderer._text(invoice,d, (paybox_x0 + mm(3), pay_y0 + mm(2.6)), "Celkem k úhradě", font=textRenderer._f10, fill=INK)
        textRenderer._text_right(invoice,d, paybox_x1 - mm(3), pay_y0 + mm(2.2), fmt_money(data.calculated_total_price), textRenderer._f10b, INK, span_tag=SpanTag.TOTAL)

        textRenderer._text(invoice,d, (paybox_x0 + mm(3), pay_y0 + mm(11.6)), "Zálohy", font=textRenderer._f10, fill=INK)
        textRenderer._text_right(invoice,d, paybox_x1 - mm(3), pay_y0 + mm(11.2), fmt_money(getattr(data, "advance_paid", 0)), textRenderer._f10b, INK)

        textRenderer._text(invoice,d, (paybox_x0 + mm(3), pay_y0 + mm(20.8)), "Zbývá uhradit [Kč]", font=textRenderer._f10, fill=INK)
        textRenderer._text_right(invoice,
            d,
            paybox_x1 - mm(3),
            pay_y0 + mm(20.1),
        ...
        textRenderer._text_center(invoice,d, (x0 + x1) / 2, sig_y + mm(3), "Razítko a podpis", textRenderer._f10, MUTED)

        # spodní texty
        foot_y = _A4_H_PX - margin_b - mm(6)
        textRenderer._text_center(invoice,d, _A4_W_PX / 2, foot_y, "Vytištěno systémem ABRA Flexi.", textRenderer._f10, MUTED)
        textRenderer._text_right(invoice,d, _A4_W_PX - margin_r, foot_y, "Stránka 1", textRenderer._f10, MUTED)
