...
from app.invoices_generator.templates.alza_invoice import alza_invoice
from app.invoices_generator.templates.general_invoice import general_invoice
from app.invoices_generator.templates.phone_invoice import phone_invoice
from app.invoices_generator.templates.post_invoice import post_invoice
from app.invoices_generator.templates.restaurant_receipt import restaurant_receipt
from app.invoices_generator.templates.store_receipt import store_receipt
from app.invoices_generator.templates.classic_invoice import classic_invoice
from app.invoices_generator.templates.modern_invoice import modern_invoice
from app.invoices_generator.templates.colorful_invoice import colorful_invoice
from app.invoices_generator.templates.compact_invoice import compact_invoice
from app.invoices_generator.templates.a_invoice import a_invoice
from app.invoices_generator.templates.simple_invoice import simple_invoice
from app.invoices_generator.templates.inverted_invoice import inverted_invoice
from app.invoices_generator.templates.random_invoice import random_invoice

from app.invoices_generator.utility.invoice_consts import *
...
class invoice_generator:
    ...
    def __generate_item(self)->invoice_item:
        ...
        ppu = random.randrange(100,10000)
    ...
    def __generate_invoice_number(self) -> str:   
        if(random.choice([True, False])):
            return f"{random.randrange(0,99999999999)}"
        else:
    ...
    def __generate_folder(self, folder: str, count: int, engine:engines = engines.DONUT) -> None:
                ...
                with open(meta_path, "a", encoding="utf-8") as f:
                    if(engine == engines.DONUT):
                        output = {
                            ...
                            "ground_truth": {
                                "gt_parse": instance.to_json(img_path, engine)
                            }
                        }
                    elif(engine == engines.LAYOUTLMv3):
                        output = {
                            "file_name": f"{cls.__name__}_{invoice_number}.png",
                            "data": instance.to_json(img_path, engine)
                        }
                    
                    f.write(json.dumps(output, ensure_ascii=False) + "\n")

                
    def generate(self, train_count:int, test_count:int, validation_count:int, engine:engines = engines.DONUT)->bool:
        
        if(train_count>0):
            self.__generate_folder("train", train_count, engine)

        if(test_count>0):
            self.__generate_folder("test", test_count, engine)

        if(validation_count>0):
            self.__generate_folder("validation", validation_count, engine)
