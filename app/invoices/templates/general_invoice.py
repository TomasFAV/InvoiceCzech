...
class general_invoice(invoice):
    ...
    def generate_img(self, output_path: str) -> bool:
        ...
        self._draw_center(d, title_center_x, title_y + self.mm(3.5),
                          f"DAŇOVÝ DOKLAD (FAKTURA) č. {self._safe(self.invoice_number)}",
                          self._f14b, self._INK)

        # Variabilní a konstantní symbol (pod titulkem)
        symbol_y = title_y + title_height + self.mm(3)
        self._draw_right(d, self._A4_W_PX - margin_r - self.mm(5), symbol_y,
                         f"Variabilní symbol: {self._safe(self.variable_symbol)}", self._f11, self._INK)
        symbol_y += self.mm(4.5)
        self._draw_right(d, self._A4_W_PX - margin_r - self.mm(5), symbol_y,
                         f"Konstantní symbol: {self._safe(self.const_symbol)}",
                         self._f11, self._INK)
        ...
        d.text((margin_l + self.mm(3), supplier_text_y), "Dodavatel:", font=self._f13b, fill=self._INK)

        d.text((margin_l + self.mm(30), supplier_text_y), f"IČ: {self._safe(self.supplier.register_id)}",
               font=self._f11, fill=self._INK)

        d.text((margin_l + self.mm(57), supplier_text_y), f"DIČ: {self._safe(self.supplier.tax_id)}",
               font=self._f11, fill=self._INK)
        supplier_text_y += self.mm(6)

        d.text((margin_l + self.mm(3), supplier_text_y), f"{self._safe(self.supplier.name)} {self.supplier.type.value}",
               font=self._f11b, fill=self._INK)
        supplier_text_y += self.mm(4)

        if hasattr(self.supplier, 'contact_name'):
            d.text((margin_l + self.mm(3), supplier_text_y), self._safe(self.supplier.contact_name),
                   font=self._f11, fill=self._INK)
            supplier_text_y += self.mm(4)

        d.text((margin_l + self.mm(3), supplier_text_y), self._safe(self.supplier.street),
               font=self._f11, fill=self._INK)
        supplier_text_y += self.mm(4)

        if hasattr(self.supplier, 'city'):
            d.text((margin_l + self.mm(3), supplier_text_y), f"{self._safe(self.supplier.zip)} {self._safe(self.supplier.city)}",
                   font=self._f11, fill=self._INK)
            supplier_text_y += self.mm(6)

        # Registrační poznámka
        if hasattr(self.supplier, 'registration_note'):
            d.text((margin_l + self.mm(3), supplier_text_y), self._safe(self.supplier.registration_note),
                   font=self._f11, fill=self._INK)
        ...
        d.text((margin_l + self.mm(3), bank_text_y), "Bankovní spojení:", font=self._f13b, fill=self._INK)
        bank_text_y += self.mm(6)

        bank_name = getattr(self.bank_account, 'bank_name', '') if hasattr(self.bank_account, 'bank_name') else ''
        bic = getattr(self.bank_account, 'BIC', '') if hasattr(self.bank_account, 'BIC') else ''
        d.text((margin_l + self.mm(3), bank_text_y), f"Banka / SWIFT: {self.bank_account.name} / {bic}",
               font=self._f10, fill=self._INK)
        bank_text_y += self.mm(4)

        d.text((margin_l + self.mm(3), bank_text_y), f"Číslo účtu: {self._safe(self.bank_account_number)}",
               font=self._f11, fill=self._INK)
        bank_text_y += self.mm(4)

        d.text((margin_l + self.mm(3), bank_text_y), f"IBAN: {self._safe(self.IBAN)}",
               font=self._f11, fill=self._INK)
        bank_text_y += self.mm(6)

        # Obchodní údaje
        d.text((margin_l + self.mm(3), bank_text_y), "Obchodní údaje:", font=self._f12b, fill=self._INK)
        bank_text_y += self.mm(4)
        d.text((margin_l + self.mm(3), bank_text_y), f"Zakázka: {self._safe(getattr(self, 'order_job', ''))}",
               font=self._f11, fill=self._INK)
        bank_text_y += self.mm(3.5)
        d.text((margin_l + self.mm(3), bank_text_y), f"Objednávka: {self._safe(getattr(self, 'order_number', ''))}",
               font=self._f11, fill=self._INK)
        bank_text_y += self.mm(3.5)
        d.text((margin_l + self.mm(3), bank_text_y),
        ...
        d.text((right_x + self.mm(3), customer_text_y), "Odběratel:", font=self._f13b, fill=self._INK)
        # customer_text_y += self.mm(6)

        d.text((right_x + self.mm(30), customer_text_y), f"IČ: {self._safe(self.customer.register_id)}",
               font=self._f11, fill=self._INK)
        # customer_text_y += self.mm(4)
        d.text((right_x + self.mm(57), customer_text_y), f"DIČ: {self._safe(self.customer.tax_id)}",
               font=self._f11, fill=self._INK)
        customer_text_y += self.mm(6)

        d.text((right_x + self.mm(3), customer_text_y), self._safe(self.customer.name),
               font=self._f12b, fill=self._INK)
        customer_text_y += self.mm(4.5)
        d.text((right_x + self.mm(3), customer_text_y), self._safe(self.customer.street),
               font=self._f11, fill=self._INK)
        customer_text_y += self.mm(4)

        if hasattr(self.customer, 'city'):
            d.text((right_x + self.mm(3), customer_text_y), f"{self._safe(self.customer.zip)} {self._safe(self.customer.city)}",
                   font=self._f11, fill=self._INK)
            customer_text_y += self.mm(6)

        # Kontakty
        d.text((right_x + self.mm(3), customer_text_y), f"Tel.: {self._safe(getattr(self.customer, 'phone', ''))}",
               font=self._f11, fill=self._INK)
        customer_text_y += self.mm(4)
        d.text((right_x + self.mm(3), customer_text_y), f"Fax: {self._safe(getattr(self.customer, 'fax', ''))}",
               font=self._f11, fill=self._INK)
        customer_text_y += self.mm(4)
        d.text((right_x + self.mm(3), customer_text_y), f"E-mail: {self._safe(getattr(self.customer, 'email', ''))}",
               font=self._f11, fill=self._INK)
        ...
        def date_row(label:str, value:str, bold:bool=False)->None:
            nonlocal dates_y
            font_val = self._f12b if bold else self._f12
            d.text((right_x + self.mm(3), dates_y), label, font=self._f11, fill=self._INK)
            self._draw_right(d, self._A4_W_PX - margin_r - self.mm(5), dates_y, self._safe(value),
                             font_val, self._INK)
            dates_y += self.mm(5.5)

        date_row("Datum splatnosti:", self.due_date, True)
        date_row("Datum vystavení:", self.issue_date, True)
        date_row("Datum uskutečnění zdanitelného plnění:", self.taxable_supply_date, True)

        payment_method = self.payment.value if hasattr(self.payment, 'value') else str(self.payment)
        date_row("Forma úhrady:", payment_method, True)
        ...
        col_ws = [0.32, 0.08, 0.12, 0.16, 0.08, 0.08, 0.16]
        ...
        for i, header_text in enumerate(headers):
            if i == 0:  # První sloupec - vlevo
                d.text((x_cols[i] + 8, baseline), header_text, font=self._f11b, fill=self._INK)
            elif i in (1, 4, 5):  # MJ, DPH, Sleva - na střed
                self._draw_center(d, x_cols[i] + col_abs[i] // 2, baseline, header_text, self._f11b, self._INK)
            else:  # Ostatní - vpravo
                self._draw_right(d, x_cols[i] + col_abs[i] - 8, baseline, header_text, self._f11b, self._INK)
        ...
        for item in self.items:
            ...
            # Vykreslení buněk
            d.text((x_cols[0] + 8, y_text), cells[0], font=self._f11, fill=self._INK)
        ...
        if hasattr(self, 'gdpr_note') and self.gdpr_note:
            draw_box(margin_l, y, table_w, row_h, border_color=self._LINE_MID)
            y_text = y + self.mm(2)
            d.text((margin_l + 8, y_text), self._safe(self.gdpr_note), font=self._f11, fill=self._INK)
            y += row_h
        ...
        self._draw_right(d, self._A4_W_PX - margin_r, y,
                         f"Součet ({currency_text}): {self._fmt_money(subtotal_without_vat)}",
                         self._f12b, self._INK)
        y += self.mm(10)
        ...
        # Nadpis rozpisu
        d.text((margin_l + 8, y + self.mm(2.5)), "Rozpis DPH v CZK:", font=self._f12b, fill=self._INK)
        ...
        for i, header in enumerate(vat_headers):
            ...
            if i == 0:
                d.text((vat_x_cols[i] + 6, vat_y_text), header, font=self._f11b, fill=self._INK)
            else:
                self._draw_right(d, vat_x_cols[i] + vat_col_abs[i] - 6, vat_y_text, header, self._f11b, self._INK)
        ...
        vat_row_h = self.mm(6)
        subtotal_base = getattr(self, "subtotal_without_vat",
                                sum(getattr(v, "vat_base", 0) for v in self.vat))
        total_vat = getattr(self, "calculated_total_vat",
                            sum(getattr(v, "vat", 0) for v in self.vat))
        total_with_vat = getattr(self, "calculated_total_price", subtotal_base + total_vat)
        ...
        for v in self.vat:
            ...
            rate_txt = f"{self._safe(getattr(v, 'label', ''))} {self._safe(v.vat_percentage)}%".strip()
            d.text((vat_x_cols[0] + 6, vat_y_text), rate_txt, font=self._f11, fill=self._INK)

            # sloupec 1: základ
            self._draw_right(d, vat_x_cols[1] + vat_col_abs[1] - 6, vat_y_text,
                             self._fmt_money(v.vat_base), self._f11, self._INK)

            # sloupec 2: DPH
            self._draw_right(d, vat_x_cols[2] + vat_col_abs[2] - 6, vat_y_text,
                             self._fmt_money(v.vat), self._f11, self._INK)

            # sloupec 3: celkem za sazbu (základ + DPH)
            self._draw_right(d, vat_x_cols[3] + vat_col_abs[3] - 6, vat_y_text,
                             self._fmt_money(v.vat_base + v.vat), self._f11, self._INK)
        ...
        vat_y_text = y + self.mm(1.5)
        d.text((vat_x_cols[0] + 6, vat_y_text), "Součet", font=self._f11b, fill=self._INK)
        self._draw_right(d, vat_x_cols[1] + vat_col_abs[1] - 6, vat_y_text,
                         self._fmt_money(subtotal_without_vat), self._f11b, self._INK)
        self._draw_right(d, vat_x_cols[2] + vat_col_abs[2] - 6, vat_y_text,
                         self._fmt_money(self.calculated_total_vat), self._f11b, self._INK)
        self._draw_right(d, vat_x_cols[3] + vat_col_abs[3] - 6, vat_y_text,
                         self._fmt_money(self.calculated_total_price), self._f11b, self._INK)

        # Vertikální linky
        for i in range(1, len(vat_x_cols)):
            d.line([(vat_x_cols[i], y), (vat_x_cols[i], y + vat_row_h)], fill=self._LINE_STRONG, width=2)

        # Celková částka k úhradě (vpravo) - větší box jako v HTML
        total_y = y - self.mm(20)
        total_box_w = self.mm(60)
        total_box_h = self.mm(15)

        draw_box(right_block_x + total_box_w//2 + self.mm(7), total_y+self.mm(2), self.mm(23), self.mm(6),
                 border_color=self._LINE_STRONG, border_width=3)

        total_price = getattr(self, 'calculated_total_price', self.calculated_total_price)
        total_text = f"Celkem k úhradě ({currency_text}):  {self._fmt_money(total_price)}"

        # Dvouřádkové zobrazení
        self._draw_center(d, right_block_x + total_box_w // 2, total_y + self.mm(3),
                          total_text, self._f12b, self._INK)
        ...
        d.text((margin_l + 8, y), self._safe(note_text), font=self._f11, fill=self._INK)
        ...
        d.text((margin_l, y), footer_text, font=self._f11b, fill=self._INK)
