...
class HeaderA(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, (x, y), text="LOGO", font=textRenderer._f16b, fill=INK)
        
        # 2. Název dokladu a doplňkové info (vpravo)
        r_align =  page_width - 2*x
        textRenderer._text(invoice, (r_align, y), text="FAKTURA - DAŇOVÝ DOKLAD", font=textRenderer._f14b, fill=INK)
        y += mm(6)
        textRenderer._text(invoice, (r_align, y), label="Číslo dokladu: ", text=f"{safe(data.invoice_number)}",
        ...
        textRenderer._text(invoice, (x, y), text=small_info, font=textRenderer._f8, fill=INK)
