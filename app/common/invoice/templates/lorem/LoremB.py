...
class LoremB(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, (x + mm(2), y + box_h + mm(1)), text="Razítko a podpis", font=textRenderer._f8, fill=MUTED)
        ...
        textRenderer._text(invoice, (stamp_x + mm(4), y + mm(8)), text="SCHVÁLENO", font=textRenderer._f8b, fill=(180, 40, 40))
        ...
        for phrase in fine_print:
            ...
            for word in words:
                ...
                if text_width(test_line, textRenderer._f8) < (max_w - mm(5)):
                    line = test_line
                else:
                    textRenderer._text(invoice, (x, y), text=line, font=textRenderer._f8, fill=MUTED)
                    y += mm(3)
                    line = word + " "
