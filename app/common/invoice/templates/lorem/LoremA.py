...
class LoremA(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, (qr_x, y + qr_size + mm(1)), text="QR PLATBA / INFO", font=textRenderer._f8b, fill=MUTED)

        # 2. Marketingový slogan
        # Omezíme šířku sloganu, aby nenarazil do QR kódu
        textRenderer._text(invoice, (x, y), text="DĚKUJEME ZA VAŠI DŮVĚRU!", font=textRenderer._f10b, fill=TMOBILE_PINK)
        y += mm(7)
        ...
        for phrase in lorem_phrases[:3]:
            ...
            for word in words:
                ...
                if text_width(test_line, textRenderer._f10) < (current_limit - mm(2)):
                    ...
                    textRenderer._text(invoice, (x, y), text=line, font=textRenderer._f10, fill=MUTED)
                    y += mm(3.5)
        ...
        textRenderer._text(invoice, (x + mm(4), y), text="Šetříme přírodu. Faktura v elektronické podobě.", font=textRenderer._f10, fill=MUTED)
