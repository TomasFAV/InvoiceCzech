...
class TotalA(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, (x, y), text="Celkem k úhrade:", font=textRenderer._f11b, fill=INK)
        y += mm(6)
        total_txt = f"{fmt_money(data.calculated_total_price)}"
        textRenderer._text(invoice, (x, y), text=total_txt, end=f"{data.currency.value}", font=textRenderer._f16b, fill=INK, span_tag=SpanTag.TOTAL) 
