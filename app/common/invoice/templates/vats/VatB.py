...
from common.utils.utilities import fmt_money, mm
from common.utils.consts import INK, MUTED, LINE_MID, SUBTLE_BG
...
class VatB(InvoiceComponent):
    ...
    def draw_scaled(textRenderer: TextRenderer, data: InvoiceData, invoice: Invoice, d: ImageDraw, x: int, y: int, **kwargs):
        ...
        ty = y + header_h * 0.18
        textRenderer._text(invoice, (col1, ty), text="Sazba", font=font, fill=MUTED)
        textRenderer._text(invoice, (col2, ty), text="Základ daně", font=font, fill=MUTED)
        textRenderer._text(invoice, (col3, ty), text="Daň", font=font, fill=MUTED)
    ...
    def draw_normal(textRenderer: TextRenderer, data: InvoiceData, invoice: Invoice, d: ImageDraw, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, (col_sazba, y + mm(1)), text="Sazba", font=textRenderer._f8b, fill=MUTED)
        textRenderer._text(invoice, (col_zaklad, y + mm(1)), text="Základ daně", font=textRenderer._f8b, fill=MUTED)
        textRenderer._text(invoice, (col_dan, y + mm(1)), text="Daň", font=textRenderer._f8b, fill=MUTED)
