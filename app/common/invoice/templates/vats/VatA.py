...
from common.utils.consts import INK
from common.utils.utilities import fmt_money
...
class VatA(InvoiceComponent):
    ...
    def draw_scaled(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, d: ImageDraw, x: int, y: int, **kwargs):
        ...
        for v in data.vat:
            ...
            textRenderer._text(invoice, (col1,y), text=v.vat_percentage, label="Sazba ", span_tag=SpanTag.O, font=scaled_f10, fill=INK)
            textRenderer._text(invoice, (col2,y), text=fmt_money(v.vat_base), label="Základ ", span_tag=SpanTag.O, font=scaled_f10, fill=INK )
            textRenderer._text(invoice, (col3,y), text=fmt_money(v.vat), label="Daň ", span_tag=SpanTag.O, font=scaled_f10, fill=INK )   
    ...
    def draw_normal(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, d: ImageDraw, x: int, y: int, **kwargs):
        ...
        for v in data.vat:
            x = x_start
            x = mm(2) + textRenderer._text(invoice, (x,y), text=v.vat_percentage, label="Sazba ", span_tag=SpanTag.O, font=textRenderer._f10, fill=INK )[0]
            x = mm(2) + textRenderer._text(invoice, (x,y), text=fmt_money(v.vat_base), label="Základ ", span_tag=SpanTag.O, font=textRenderer._f10, fill=INK )[0]   
            x = mm(2) + textRenderer._text(invoice, (x,y), text=fmt_money(v.vat), label="Daň ", span_tag=SpanTag.O, font=textRenderer._f10, fill=INK )[0]
