...
class info_b(InvoiceComponent):
    ...
    def render(textRenderer:TextRenderer, data:InvoiceData, invoice:Invoice, x: int, y: int, **kwargs):
        ...
        textRenderer._text(invoice, (x + padding, curr_y), label="Doklad č.:", 
        ...
        for i, (attr, lab, tag) in enumerate(fields):
            ...
            if val:
                ...
                textRenderer._text(invoice, (field_x, field_y), text=lab, font=textRenderer._f12, fill=MUTED)
                textRenderer._text(invoice, (field_x, field_y + mm(3.5)), text=val, font=textRenderer._f14b, fill=INK, span_tag=tag)
        ...
        textRenderer._text(invoice, (x + padding, curr_y), text="Datum splatnosti:", font=textRenderer._f14, fill=INK)
        textRenderer._text(invoice, (x + width - 4*padding, curr_y), text=safe(data.due_date), 
