...
from common.utils.utilities import fit_line_bounding_box_font, fit_text_bounding_box_font, fmt_money, text_width
...
class InvoiceRenderer:
    ...
    def render_text(self, words:List[str], area:tuple[int,int,int,int], invoice:Invoice):
        ...
        for word in words:
            ...
            if(word_end_x > end_x):
                x = area[0]
                y += line_height + space
            self.text_renderer._text(invoice, poss=(x,y), text=word, fill=draw_color, font=font, span_tag=SpanTag.O)
    ...
    def render_spans(self, data:InvoiceData, invoice:Invoice):
        ...
        for span in spans_copy:
            ...
            self.text_renderer._text(invoice, (span.b_box[0], span.b_box[1]),
    ...
    def enhance(self, invoice_data:InvoiceData, invoice:Invoice):
