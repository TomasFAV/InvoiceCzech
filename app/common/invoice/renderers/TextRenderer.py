...
from common.utils.consts import _A4_H_PX, _A4_W_PX, INK
from common.utils.utilities import load_font, mm, text_width
from common.invoice.models.Invoice import Invoice
from common.invoice.renderers.InvoiceTextParaphraser import InvoiceTextParaphraser
from common.data.invoice_consts import fonts
...
class TextRenderer:
    ...
    def _text(self, invoice:Invoice, poss: Tuple[float, float], text: str, font: FreeTypeFont, fill:Tuple[int, int, int],
            label:str|None = None, end:str|None = None, span_tag:SpanTag = SpanTag.O, must_have_same_width:bool = False) -> tuple[float, int|None]:
        
        draw = ImageDraw.ImageDraw(invoice.image)
    ...
    def _text_right(self, invoice:Invoice, x_right: float, y: float, text: str, font: FreeTypeFont, fill: tuple[int, int, int], span_tag: SpanTag = SpanTag.O,
    label: str | None = None, end: str | None = None, must_have_same_width:bool = False) -> tuple[float, int|None]:
        ...
        for t, tg in parts:
            x, _ = self._text(invoice, (x, y), text=t, font=font, fill=fill, span_tag=tg, must_have_same_width=must_have_same_width)
            if(tg != SpanTag.O):
                span_index = _

        return (x, span_index)

    def _text_center(self,invoice:Invoice, x_center: float, y: float, text: str, font: FreeTypeFont, fill: tuple[int, int, int] = INK,
        ...
        for t, tg in parts:
            x, _ = self._text(invoice, (x, y), t, font=font, fill=fill, span_tag=tg, must_have_same_width=must_have_same_width)
            if(tg != SpanTag.O):
                span_index = _
